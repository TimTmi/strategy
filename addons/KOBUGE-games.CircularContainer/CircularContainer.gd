...
## Properties ##
@export var _force_squares = false
@export var _force_expand = false
@export var _start_angle = 0
@export var _percent_visible = 1
@export var _appear_at_once = false
@export var _allow_node2d = false
@export var _start_empty = false
...
func _set(property, value):
	...
	elif property == "arrange/force_expand": set_force_expand(value)
...
func _default_animator(node, container_center, target_pos, time):
	if node is Control:
		node.set_position(target_pos - node.size * 0.5)
	else:
		node.set_position(target_pos - node.size * 0.5)
...
func _get_filtered_children():
	...
	while i > 0:
		i -= 1
		var keep = false
		if (children[i] is Control) or (_allow_node2d and children[i] is Node2D):
			keep = true
...
func _get_child_stretch_ratio(child):
	...
	elif child is Node2D or _force_expand:
		return 1
	else:
		return 0
...
func _get_max_angle_for_diagonal(diagonal, radius):
	...
	return PI if fit_length > radius else asin(fit_length / radius) * 2
